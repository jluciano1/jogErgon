CREATE OR REPLACE 
PROCEDURE TGRJ_CARREGA_CAP_CARGA(P_ARQ IN BLOB, --default fileUpload
...
IS
  ...
  begin
    -- Aqui o sistema أ© executado via Web
    ...
    IF INSTR(p_arquivo,'\\') > 0 or INSTR(p_arquivo,'/')> 0 then
      ERGON_ERRO_PACK.TRATA_ERRO (99,'Na execuأ§أ£o via Web do sistema somente deve ser informado o nome do arquivo, sem diretório.' ||
             '"'||p_arquivo||'"' );
    end if;
  ...
  BEGIN
    ...
    begin
      ...
      --
      -- Colocando espaأ§o para o arquivo ter mais ZERO bytes se nأ£o ocorre erro no Forms 10g ao fazer UPLOAD
      --
...
BEGIN
  ...
  IF p_acao IS NULL THEN
    p_mens := 'A aأ§أ£o deve ser informada';
    RETURN;
  ELSIF p_acao NOT IN (1, 2, 3) THEN
    p_mens := 'Valor invأ،lido informado para a aأ§أ£o';
    RETURN;
  END IF;
  ...
  begin
    ...
    select ID_TIPO_ROT
     ...
     WHERE SIGLA = 'Carga Capacitações Carga';

  EXCEPTION
    when no_data_found then
      insert into HAD_TIPO_ROTINA_EXEC (SIGLA,DESCRICAO) values ('Carga Capacitações Carga','Carga Capacitações Carga');
  ...
  if p_acao = 3 then
    ...
    v_arq:= 'DECLARE'||chr(10)||
            'BEGIN'||chr(10)||
              'TGRJ_PACK_CARGA_CAP.DESFAZ_CARGA(''' ||
                                                    v_nome            || ''',''' ||
                                                    p_tipo            || ''',''' ||                                                    
                                                    P_CAMINHO         || ''',' ||
                                                    P_LAYOUT          || ',' ||
