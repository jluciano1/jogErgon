CREATE OR REPLACE
PACKAGE BODY TGRJ_PACK_CARGA_PREV IS
  v_chave number;
...
--  v_chave_grupo_eleito number := null;
  ...
  col_tipo          carga_colunas%rowtype;
  col_rubrica       carga_colunas%rowtype;
  col_complemento   carga_colunas%rowtype;
  col_valor         carga_colunas%rowtype;
  col_correcao      carga_colunas%rowtype;
  col_info          carga_colunas%rowtype;
  col_responsavel   carga_colunas%rowtype;
  col_observacao    carga_colunas%rowtype;
  ...
  */
  Procedure gera_relatorios(P_CAMINHO         IN VARCHAR2
    ...
    cursor cur_rejeitados is
      select linha,erro,texto_linha from carga_fol_movimentos_temp
      where chave=v_chave and erro is not null
      order by linha;

    cursor cur_aceitos is
      select linha,numfunc,numvinc,dtini,dtfim,rubrica,valor,tipo,complemento,observacao,emp_codigo,
    ...
    begin
      ...
      loop
       ...
       err_msg:=conv_linha(v_linha, v_modo_carga, r,v_oper);
        ...
        begin
          insert into carga_fol_movimentos_temp(chave,linha,
              numfunc,numvinc,dtini,dtfim,tipo,rubrica,complemento,
              valor,observacao,flex_campo_01,flex_campo_02,flex_campo_03,flex_campo_04,flex_campo_05,texto_linha, emp_codigo)
          values(v_chave,num_linha,
                 r.numfunc,r.numvinc,r.dtini,r.dtfim, r.tipo,r.rubrica,r.complemento,r.valor, 
                 r.observacao,r.flex_campo_01,r.flex_campo_02,r.flex_campo_03,r.flex_campo_04,r.flex_campo_05,v_linha, r.emp_codigo);
        exception
      ...
      exception
        when erro_linha_maior_2000 then          
          insert into carga_fol_movimentos_temp(chave,linha,rubrica,erro)
        ...
        when erro_conversao then
          ...
          insert into carga_fol_movimentos_temp(chave,linha,rubrica,erro,texto_linha)
...
--      return(false);
  ...
  */
  ...
  IS
    ...
    ra                    fol_movimentos%rowtype; -- variável auxiliar para guardar vantagens alteradas
    v_aux                 varchar2(100);
 ...
 --   where sis='C_Ergon' and trans='RJadm00063';
    --
    -- Sempre irá auditar
    --
    v_logging := 'S';
    ...
    if v_modo_carga='I' then
          begin
            begin
              --
              --
              -- Verificar se a rubrica do arquivo texto foi cadastrado no sistema -----
              select count(1) into v_rubrica
                from rubricas
                where rubrica=r.rubrica;
                
              --
              if v_rubrica = 0 then
                err_msg:='Rubrica : '||r.rubrica||' Não cadastrado no SIGRH';
                grava_mensagem(v_chave_log, err_msg);
                raise erro_vantagem;
              end if;                
              --
              ...
              if v_modo_carga='I' then
                ...
                --
                -- Pegar o proximo numero de movimento para o Servidor
                select nvl(max(movimento),0) + 1 
                  into v_movimento
                  from fol_movimentos
                  where numfunc = r.numfunc
                    and numvinc = r.numvinc;  
                --
                -- Pegar a usuario / responsavel pelo processamento
                v_responsavel := flag_pack.get_usuario;
                --
                insert into fol_movimentos(numfunc,numvinc,movimento,dtini,dtfim,tipo,rubrica,complemento,
                                           valor,responsavel,observacao,emp_codigo,flex_campo_01,flex_campo_02,flex_campo_03,flex_campo_04,flex_campo_05)
                values(r.numfunc,r.numvinc,v_movimento,r.dtini,r.dtfim,r.tipo,r.rubrica,r.complemento,
--                       r.valor,v_responsavel,r.observacao,v_emp_codigo,r.flex_campo_01,r.flex_campo_02,r.flex_campo_03,r.flex_campo_04,r.flex_campo_05);
                       r.valor,v_responsavel,r.observacao,r.emp_codigo,r.flex_campo_01,r.flex_campo_02,r.flex_campo_03,r.flex_campo_04,r.flex_campo_05);              
--
                --
          ...
          exception
            ...
            when DUP_VAL_ON_INDEX then
              ...
              if v_logging='S' then
                ...
                v_mens := 'Erro na linha '||to_char(num_linha)||'. '||
                          'A Rubrica '||r.rubrica || ' para o funcionário '
            ...
            when others then
              ...
              if v_logging='S' then
                --
                v_mens := 'Erro na linha '||to_char(num_linha)||'. '||
                          'Erro ao inserir a rubrica '||r.rubrica || ' para o funcionário '
                           ||pack_ergon.get_ident_func(r.numfunc,r.numvinc)
        ...
        -- Insere registro na tabela CARGA_FOL_MOVIMENTOS_TEMP
        begin
          insert into carga_fol_movimentos_temp(chave,linha,numfunc,numvinc,dtini,dtfim,rubrica,complemento,tipo,
              valor,observacao,flex_campo_01,flex_campo_02,flex_campo_03,flex_campo_04,flex_campo_05,texto_linha,emp_codigo)
          values(v_chave,num_linha,
                 r.numfunc,r.numvinc,r.dtini,r.dtfim,r.rubrica,r.complemento,r.tipo, r.valor,r.observacao,
                 r.flex_campo_01,r.flex_campo_02,r.flex_campo_03,r.flex_campo_04,r.flex_campo_05,v_linha,r.emp_codigo);
        exception
          when others then
            null;
        end;
...
--      return(false);
 ...
 exception
  ...
  begin
    ...
    col_tipo.numero:=null;
    col_rubrica.numero:=null;
    col_complemento.numero:=null;
    col_valor.numero:=null;
    col_correcao.numero:=null;
    col_responsavel.numero:=null;
    col_observacao.numero:=null;
    ...
    col_tipo.coluna:=null;
    col_rubrica.coluna:=null;
    col_complemento.coluna:=null;
    col_valor.coluna:=null;
    col_correcao.coluna:=null;
    col_responsavel.coluna:=null;
    col_observacao.coluna:=null;
    ...
    col_complemento.valor:=null;
    col_valor.valor:=null;
    col_correcao.valor:=null;
    col_responsavel.valor:=null;
    col_observacao.valor:=null;
    ...
    col_tipo.formato:=null;
    col_rubrica.formato:=null;
    col_complemento.formato:=null;
    col_valor.formato:=null;
    col_correcao.formato:=null;
    col_responsavel.formato:=null;
    col_observacao.formato:=null;
    ...
    col_complemento.ordem:=null;
    col_valor.ordem:=null;
    col_correcao.ordem:=null;
    col_responsavel.ordem:=null;
    col_observacao.ordem:=null;
...
--         raise;
    ...
    exception
      when no_data_found then
        ...
        exception when no_data_found then
         err_msg:='Falta a coluna RUBRICA no layout do arquivo.';
         raise;
        end;
      when others then raise;
    end;
    ...
    begin
      ...
      col_valor.formato:=formato_numerico(col_valor.formato);
      tem_valor:=true;
    exception
      when no_data_found then 
        col_valor.numero:=null;
    ...
    begin
      select * into col_tipo from carga_colunas
      where numero=p_numero and upper(coluna)='TIPO';
     -- tem_tipo:=true;
    exception
      when no_data_found then 
        col_tipo.numero:=null;
    ...
    -- COMPLEMENTO
    begin
      select * into col_complemento from carga_colunas
      where numero=p_numero and upper(coluna)='COMPLEMENTO';
    ...
    -- OBSERVACAO
    begin
      select * into col_observacao from carga_colunas
    ...
    --
    if col_tipo.numero is not null and col_tipo.valor is null and 
       ((car_delimitador is null and (col_tipo.inicio is null or col_tipo.fim is null))
        or (car_delimitador is not null and col_tipo.ordem is null)) then
      err_msg:='O valor/posição da coluna TIPO não está definido corretamente no layout.';
      raise value_error;
    end if; 
    --
    if col_complemento.numero is not null and col_complemento.valor is null and 
       ((car_delimitador is null and (col_complemento.inicio is null or col_complemento.fim is null))
        or (car_delimitador is not null and col_complemento.ordem is null)) then
      err_msg:='O valor/posição da coluna COMPLEMENTO não está definido corretamente no layout.';
      raise value_error;
    end if;
    --
    if col_observacao.numero is not null and col_observacao.valor is null and 
       ((car_delimitador is null and (col_observacao.inicio is null or col_observacao.fim is null))
        or (car_delimitador is not null and col_observacao.ordem is null)) then
   ...
   * Retorna NULL se conversão OK, retorna mensagem em caso de erro.
   ************************************************************************************/
   function conv_linha(p_linha in varchar2, p_modo_carga in varchar2, p_rec out fol_movimentos%rowtype, p_oper out varchar2) return varchar2 is
   ...
   v_rubrica        varchar(20);
   v_valor          varchar(20);
   v_tipo           varchar(20);
   v_complemento    varchar(20);
   v_observacao     varchar(20);
   v_flex_campo_01     varchar(20);
   v_flex_campo_02     varchar(20);
   v_flex_campo_03     varchar(20);
   v_flex_campo_04     varchar(20);
   v_flex_campo_05     varchar(20);
   v_obs            varchar2(2000);
   ...
   begin
     ...
     if car_delimitador is null then
       ...
       begin
         ...
         -- RUBRICA
         if col_rubrica.numero is not null then
...
*/
           ...
           err_msg := 'Campo RUBRICA devem ser fornecidos.';
           return (err_msg); 
         end if;
         --
         -- DTINI
         ...
         -- TIPO
         if col_tipo.numero is not null then
           if col_tipo.valor is null then
             v_tipo:=substr(p_linha,col_tipo.inicio,col_tipo.fim-col_tipo.inicio+1);
           else
             v_tipo:=col_tipo.valor;
           end if;
         end if;
         -- COMPLEMENTO
         if col_complemento.numero is not null then
           if col_complemento.valor is null then
             v_complemento:=substr(p_linha,col_complemento.inicio,col_complemento.fim-col_complemento.inicio+1);
           else 
             v_complemento:=col_complemento.valor;
           end if;
         end if;
         -- OBSERVACAO
         if col_observacao.numero is not null then
           if col_observacao.valor is null then
             v_observacao:=substr(p_linha,col_observacao.inicio,col_observacao.fim-col_observacao.inicio+1);
           else
             v_observacao:=col_observacao.valor;
           end if;
         end if;
         -- EMP_CODIGO
...
--             err_msg:='O tamanho do campo '||to_char(i)||' é maior que 60 caracteres';
         ...
         begin
           ...
           v_tipo:=nvl(col_tipo.valor,t_campo(nvl(col_tipo.ordem,0)));
           v_complemento:=nvl(col_complemento.valor,t_campo(nvl(col_complemento.ordem,0)));
           v_observacao:=nvl(col_observacao.valor,t_campo(nvl(col_observacao.ordem,0)));
     ...
     begin
       p_rec.rubrica:=ltrim(rtrim(v_rubrica));
     exception
       when others then return('Valor inválido para RUBRICA');
     end;
     begin
     ...
     begin
       ...
       p_rec.valor:=to_number(v_valor);  --,col_valor.formato);
     exception
       when others then return('Valor inválido para VALOR');
     end;
     begin
       p_rec.tipo:=ltrim(rtrim(v_tipo));
     exception
       when others then return('Valor inválido para TIPO');
     end;
     begin
       p_rec.complemento:=ltrim(rtrim(v_complemento));
     exception
       when others then return('Valor inválido para COMPLEMENTO');
     end;
     begin
       p_rec.observacao:=ltrim(rtrim(v_observacao));
     exception
