CREATE OR REPLACE 
PROCEDURE TGRJ_CARREGA_FREQ(P_ARQ IN BLOB, --default fileUpload
...
BEGIN
  ...
  begin
    ...
    select ID_TIPO_ROT
     ...
     WHERE SIGLA = 'Carga Frequência Customizada';

  EXCEPTION
    when no_data_found then
      insert into HAD_TIPO_ROTINA_EXEC (SIGLA,DESCRICAO) values ('Carga Frequência Customizada','Carga Frequência Customizada');
  ...
  if p_acao = 3 then
    ...
    v_arq:= 'DECLARE'||chr(10)||
            'BEGIN'||chr(10)||
              'TGRJ_PACK_CARGA_FREQ.DESFAZ_CARGA(''' ||
                                                    v_nome            || ''',''' ||
                                                    p_tipo            || ''',''' ||                                                    
                                                    P_CAMINHO         || ''',' ||
                                                    P_LAYOUT          || ',' ||
