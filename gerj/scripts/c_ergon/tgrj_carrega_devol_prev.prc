CREATE OR REPLACE 
PROCEDURE TGRJ_CARREGA_DEVOL_PREV(P_ARQ IN BLOB, --default fileUpload
...
BEGIN
  ...
  begin
    ...
    select ID_TIPO_ROT
     ...
     WHERE SIGLA = 'Carga Devolução Previdenciaria';

  EXCEPTION
    when no_data_found then
      insert into HAD_TIPO_ROTINA_EXEC (SIGLA,DESCRICAO) values ('Carga Devolução Previdenciaria','Carga Devolução Previdenciaria');
  ...
  if p_acao = 3 then
    ...
    v_arq:= 'DECLARE'||chr(10)||
            'BEGIN'||chr(10)||
              'TGRJ_PACK_CARGA_PREV.DESFAZ_CARGA(''' ||
                                                    v_nome            || ''',''' ||
                                                    p_tipo            || ''',''' ||                                                    
                                                    P_CAMINHO         || ''',' ||
                                                    P_LAYOUT          || ',' ||
