CREATE OR REPLACE
PACKAGE BODY TGRJ_PACK_CARGA_FREQ IS
  --v_chave number;
...
--  v_chave_grupo_eleito number := null;
  ...
  col_tipofreq 			carga_colunas%rowtype;
  col_codfreq 			carga_colunas%rowtype;
  col_quantidade 		carga_colunas%rowtype;
...
*/
  ...
  */
  Procedure gera_relatorios(P_CAMINHO         IN VARCHAR2
                            ...
                            ,P_CHAVE          IN carga_freq_temp.CHAVE%TYPE
    ...
    v_chave          carga_freq_temp.CHAVE%TYPE;

    v_modo_carga     VARCHAR2(1);

    cursor cur_rejeitados is
      select linha,erro,texto_linha from carga_freq_temp
      where chave=v_chave and erro is not null
      order by linha;

    cursor cur_aceitos is
      select linha,numfunc,numvinc,dtini,dtfim,tipofreq,codfreq,quantidade,pontpubl,obs,
						 pontlei,emp_codigo,hora_entrada,hora_saida,id_reg,texto_linha from carga_freq_temp
...
--       end if;
  ...
  */
  function calcula_totais(p_relat out varchar2
                         ,p_check boolean
                         ,P_CHAVE CARGA_FREQ_TEMP.CHAVE%TYPE) return boolean is
  ...
  begin
    ...
    from CARGA_FREQ_TEMP
    where chave=v_chave and erro is null;
p_relat := 'v_numero_funcionarios: '||v_numero_funcionarios;
return true;
    -- Conta tipos de frequências
    select count(distinct tipofreq) into v_qtde_tipo_freq
    from CARGA_FREQ_TEMP
    where chave=v_chave and erro is null;

    -- Se houver só um tipo de frequência, lê o tipo
    if v_qtde_tipo_freq = 1 then
      select distinct tipofreq into v_frequencia_encontrada
      from carga_freq_temp
      where chave=v_chave and erro is null;
    ...
    -- Conta códigos de frequências diferentes da especificada
    select count(distinct codfreq) into v_qtde_cod_freq
    from carga_freq_temp
    where chave=v_chave and erro is null;
...
--    end if;
    ...
    if p_check = true then
      ...
      if v_qtde_tipo_freq>1 then
        ...
        v_texto:=v_texto||'Frequencia: ('||ltrim(to_char(v_qtde_cod_freq))||' códigos diferentes)'||chr(10);
        ...
        if v_qtde_cod_freq > 1 then
          ...
          v_texto:=v_texto||'Frequencia: de mesmo código '||chr(10);
        end if;
  ...
  *******************************************************************************
  * FUNCTION LE_ARQUIVO
  *   Função carregar o arquivo de carga para a tabela temporária CARGA_FREQ_TEMP,
  ...
  return boolean is
    ...
    begin
      ...
      loop
        ...
        begin
          insert into carga_freq_temp(CHAVE,
																		  LINHA,
																		  NUMFUNC,
																		  NUMVINC,
																		  DTINI,
      ...
      exception
        when erro_linha_maior_2000 then
          insert into carga_freq_temp(chave,linha,codfreq,erro)
        ...
        when erro_conversao then
          ...
          insert into carga_freq_temp(chave,linha,codfreq,erro,texto_linha)
...
--    end if;
  ...
  */
  ...
  begin
    ...
    --
     v_chave_log := log_pack.insere_log_header('VERIFICANDO O ARQUIVO DE CARGA DE FREQUÊNCIAS',
    ...
    grava_mensagem (v_chave_log, 'Apagando os registros da tabela temporária CARGA_FREQ_TEMP.');
    --
    -- Apaga registros da tabela temporária
    --
    LOOP
      --
      DELETE CARGA_FREQ_TEMP
      WHERE CHAVE = v_chave
        AND ROWNUM <= 100;
      --
    ...
    --
    COMMIT;
    grava_mensagem (v_chave_log, 'Fim da exclusão dos registros da tabela temporária CARGA_FREQ_TEMP.');
    --
...
--      return(false);
  ...
  *******************************************************************************
  */
  FUNCTION Carga_arquivo (P_CHAVE      CARGA_FREQ_TEMP.CHAVE%TYPE
                         ,P_CAMINHO    IN VARCHAR2
  ...
  IS
    ...
    ra                    frequencias%rowtype; -- variável auxiliar para guardar vantagens alteradas
    v_aux                 varchar2(100);
...
--            :dialog.mensagem:='Efetuando carga: linha '||to_char(num_linha);
        ...
        if v_modo_carga='E' then
          ...
          begin
            update frequencias
            set dtfim=r.dtfim
            where numfunc=r.numfunc and numvinc=r.numvinc and codfreq=r.codfreq and
            ...
            if sql%rowcount<>0 then
              ...
              from frequencias
              where numfunc=r.numfunc and numvinc=r.numvinc and codfreq=r.codfreq;
              if v_cont=0 then
          ...
          begin
            begin
              ...
              select count(1) into v_frequencia
                from codigos_freq_ cf
               where cf.codigo = r.codfreq;

              --
              if v_frequencia = 0 then
              	err_msg:='Frequência : '||v_nomecodfreq||' Não cadastrado no SIGRH';
                grava_mensagem(v_chave_log, err_msg);
              	raise erro_frequencia;
              end if;
            	--

              if v_cont=0 or v_modo_carga='I' then
                insert into frequencias(numfunc,numvinc,dtini,dtfim,tipofreq,codfreq,obs,flex_campo_03)
                values(r.numfunc,r.numvinc,r.dtini,r.dtfim,r.tipofreq,r.codfreq,r.obs,v_chave_log);
--
                --
                ...
                --
                  ...
                  if v_cont > 1 then
                    ...
                    -- Atualiza registro na tabela FREQUENCIAS
                    update frequencias
                    set dtfim=r.dtfim, obs=r.obs
                    where numfunc=r.numfunc and numvinc=r.numvinc and codfreq=r.codfreq and
                          to_char(dtini,'ddmmyyyy')=to_char(r.dtini,'ddmmyyyy');
                    --
          ...
          exception
            ...
            when DUP_VAL_ON_INDEX then
              ...
              if v_logging='S' then
                ...
                v_mens := 'Erro na linha '||to_char(num_linha)||'. '||
                          'A frequência '||v_nomecodfreq || ' para o funcionário '
            ...
            when others then
              ...
              if v_logging='S' then
                --
                v_mens := 'Erro na linha '||to_char(num_linha)||'. '||
                          'Erro ao inserir a frequência '||v_nomecodfreq || ' para o funcionário '
                           ||pack_ergon.get_ident_func(r.numfunc,r.numvinc)
          ...
          begin
            ...
            if v_cont = 0 then
              ...
              raise erro_carga;
                ...
                where numfunc=r.numfunc and numvinc=r.numvinc and
                      tipofreq=r.tipofreq and codfreq=r.codfreq and
                ...
                if sql%rowcount=0 then
                  ...
                  from frequencias
                  where numfunc=r.numfunc and numvinc=r.numvinc and codfreq=r.codfreq and
                       tipofreq=r.tipofreq and to_char(dtini, 'ddmmyyyy')= to_char(r.dtini, 'ddmmyyyy');
                  if v_cont>0 then
        ...
        -- Insere registro na tabela CARGA_FREQ_TEMP
        begin
          insert into carga_freq_temp(chave,linha,numfunc,numvinc,dtini,dtfim,tipofreq,codfreq,obs,texto_linha)
               values(v_chave,num_linha,r.numfunc,r.numvinc,r.dtini,r.dtfim,r.tipofreq,r.codfreq,r.obs,v_linha);
        exception
          when others then
            null;
        end;
      exception
        ...
        when erro_conversao then
          err_msg := TRUNCA_SQLERRM(err_msg);
          insert into carga_freq_temp(chave,linha,codfreq,erro,texto_linha)
          values(v_chave,num_linha,r.codfreq,err_msg,v_linha);
          v_cont_erros:=v_cont_erros+1;
...
--      commit;
    ...
    if v_logging='S' then
      ...
      if v_modo_carga='D' then
        --
        v_mens := 'Foram removidos um total de '||v_linhas_removidas||' frequências da carga';
        --
      elsif v_modo_carga='E' then
        --
        v_mens := 'Foram encerrados um total de '||v_linhas_alteradas||' frequências da carga';
        --
      elsif v_modo_carga='I' or v_modo_carga='A' then
        --
        v_mens := 'Foram carregados um total de '||v_linhas_inseridas||' frequências e '
                  ||'alterados um total de '||v_linhas_alteradas||' frequências';
        --
      end if;
  ...
  */
  ...
  begin
    ...
    -- Seta as variáveis internas da package
    select nvl(max(chave),0)+1 into v_chave from carga_freq_temp;
...
--      return(false);
 ...
 exception
  ...
  begin
    ...
    col_tipofreq.numero:=null;
    col_codfreq.numero:=null;
    col_quantidade.numero:=null;
    ...
    col_tipofreq.coluna:=null;
    col_codfreq.coluna:=null;
    col_quantidade.coluna:=null;
    ...
    col_tipofreq.valor:=null;
    col_codfreq.valor:=null;
    col_quantidade.valor:=null;
    ...
    col_tipofreq.formato:=null;
    col_codfreq.formato:=null;
    col_quantidade.formato:=null;
    ...
    col_tipofreq.ordem:=null;
    col_codfreq.ordem:=null;
    col_quantidade.ordem:=null;
...
--         raise;
    ...
    begin
      select * into col_tipofreq from carga_colunas
      where numero=p_numero and upper(coluna)='TIPOFREQ';
    exception
      when no_data_found then
        if p_modo_carga<>'E' then
          err_msg:='Falta a coluna TIPOFREQ no layout do arquivo.';
          raise;
        end if;
      when others then raise;
    end;
    ...
    --
    begin
      select * into col_codfreq from carga_colunas
      where numero=p_numero and upper(coluna)='CODFREQ';
    exception
      when no_data_found then
        col_codfreq.numero:=null;
      when others then raise;
    end;
    if col_codfreq.coluna is null then
         err_msg:='Falta a coluna CODFREQ no layout do arquivo.';
         return err_msg;
    end if;
    --
    ...
    begin
      select * into col_quantidade from carga_colunas
      where numero=p_numero and upper(coluna)='QUANTIDADE';
      tem_quantidade:=true;
    exception
      when no_data_found then
        col_quantidade.numero:=null;
    ...
    begin
      select * into col_hora_entrada from carga_colunas
      where numero=p_numero and upper(coluna)='HORA_ENTRADA';
      tem_hora_entrada:=true;
    exception
      when no_data_found then
        col_hora_entrada.numero:=null;
        tem_hora_entrada:=false;
      when others then raise;
    end;

    begin
      select * into col_hora_saida from carga_colunas
      where numero=p_numero and upper(coluna)='HORA_SAIDA';
      tem_hora_saida:=true;
    exception
      when no_data_found then
        col_hora_saida.numero:=null;
        tem_hora_saida:=false;
      when others then raise;
    end;

     begin
      select * into col_id_reg from carga_colunas
      where numero=p_numero and upper(coluna)='ID_REG';
      tem_id_reg:=true;
    exception
      when no_data_found then
        col_id_reg.numero:=null;
    ...
    if col_tipofreq.numero is not null and col_tipofreq.valor is null and
       ((car_delimitador is null and (col_tipofreq.inicio is null or col_tipofreq.fim is null))
        or (car_delimitador is not null and col_tipofreq.ordem is null)) then
      err_msg:='O valor/posição da coluna TIPOFREQ não está definido corretamente no layout.';
      raise value_error;
    end if;
    if col_codfreq.numero is not null and col_codfreq.valor is null and
       ((car_delimitador is null and (col_codfreq.inicio is null or col_codfreq.fim is null))
        or (car_delimitador is not null and col_tipofreq.ordem is null)) then
      err_msg:='O valor/posição da coluna CODFREQ não está definido corretamente no layout.';
      raise value_error;
    end if;
    if col_quantidade.numero is not null and col_quantidade.valor is null and
       ((car_delimitador is null and (col_quantidade.inicio is null or col_quantidade.fim is null))
        or (car_delimitador is not null and col_quantidade.ordem is null)) then
      err_msg:='O valor/posição da coluna QUANTIDADE não está definido corretamente no layout.';
      raise value_error;
    end if;
    if col_pontpubl.numero is not null and col_pontpubl.valor is null and
       ((car_delimitador is null and (col_pontpubl.inicio is null or col_pontpubl.fim is null))
        or (car_delimitador is not null and col_pontpubl.ordem is null)) then
      err_msg:='O valor/posição da coluna PONTPUBL não está definido corretamente no layout.';
      raise value_error;
    end if;
    ...
    if col_pontlei.numero is not null and col_pontlei.valor is null and
       ((car_delimitador is null and (col_pontlei.inicio is null or col_pontlei.fim is null))
        or (car_delimitador is not null and col_pontlei.ordem is null)) then
      err_msg:='O valor/posição da coluna PONTLEI não está definido corretamente no layout.';
      raise value_error;
    end if;
    ...
    if col_hora_entrada.numero is not null and col_hora_entrada.valor is null and
       ((car_delimitador is null and (col_hora_entrada.inicio is null or col_hora_entrada.fim is null))
        or (car_delimitador is not null and col_hora_entrada.ordem is null)) then
      err_msg:='O valor/posição da coluna HORA_ENTRADA não está definido corretamente no layout.';
      raise value_error;
    end if;
    if col_hora_saida.numero is not null and col_hora_saida.valor is null and
       ((car_delimitador is null and (col_hora_saida.inicio is null or col_hora_saida.fim is null))
        or (car_delimitador is not null and col_hora_saida.ordem is null)) then
      err_msg:='O valor/posição da coluna HORA_SAIDA não está definido corretamente no layout.';
      raise value_error;
    end if;
    if col_ID_REG.numero is not null and col_ID_REG.valor is null and
       ((car_delimitador is null and (col_ID_REG.inicio is null or col_ID_REG.fim is null))
        or (car_delimitador is not null and col_ID_REG.ordem is null)) then
      err_msg:='O valor/posição da coluna ID_REG não está definido corretamente no layout.';
      raise value_error;
    end if;
   ...
   * Retorna NULL se conversão OK, retorna mensagem em caso de erro.
   ************************************************************************************/
   function conv_linha(p_linha in varchar2, p_modo_carga in varchar2, p_rec out frequencias%rowtype, p_oper out varchar2) return varchar2 is
   ...
   begin
     ...
     if car_delimitador is null then
       ...
       begin
         ...
         -- TIPOFREQ
         if col_tipofreq.numero is not null then
           if col_tipofreq.valor is null then
             v_tipofreq:=substr(p_linha,col_tipofreq.inicio,col_tipofreq.fim-col_tipofreq.inicio+1);
           else
             v_tipofreq:=col_tipofreq.valor;
           end if;
         end if;
         -- CODFREQ
         if col_codfreq.numero is not null then
           if col_codfreq.valor is null then
             v_codfreq:=substr(p_linha,col_codfreq.inicio,col_codfreq.fim-col_codfreq.inicio+1);
           else
             v_codfreq:=col_codfreq.valor;
           end if;
         end if;

         -- QUANTIDADE
         if col_quantidade.numero is not null then
           if col_quantidade.valor is null then
             v_quantidade:=substr(p_linha,col_quantidade.inicio,col_quantidade.fim-col_quantidade.inicio+1);
           else
             v_quantidade:=col_quantidade.valor;
           end if;
         end if;
         -- PONTPUBL
         if col_pontpubl.numero is not null then
           if col_pontpubl.valor is null then
             v_pontpubl:=substr(p_linha,col_pontpubl.inicio,col_pontpubl.fim-col_pontpubl.inicio+1);
           else
             v_pontpubl:=col_pontpubl.valor;
           end if;
         end if;
         -- OBS
         ...
         -- PONTLEI
         if col_pontlei.numero is not null then
           if col_pontlei.valor is null then
             v_pontlei:=substr(p_linha,col_pontlei.inicio,col_pontlei.fim-col_pontlei.inicio+1);
           else
             v_pontlei:=col_pontlei.valor;
           end if;
         end if;
         -- EMP_CODIGO
         ...
         -- HORA_ENTRADA
         if col_hora_entrada.numero is not null then
           if col_hora_entrada.valor is null then
             v_hora_entrada:=substr(p_linha,col_hora_entrada.inicio,col_hora_entrada.fim-col_hora_entrada.inicio+1);
           else
             v_hora_entrada:=col_hora_entrada.valor;
           end if;
         end if;
         -- HORA_SAIDA
         if col_hora_saida.numero is not null then
           if col_hora_saida.valor is null then
             v_hora_saida:=substr(p_linha,col_hora_saida.inicio,col_hora_saida.fim-col_hora_saida.inicio+1);
           else
             v_hora_saida:=col_hora_saida.valor;
           end if;
         end if;
         -- ID_REG
         if col_id_reg.numero is not null then
           if col_id_reg.valor is null then
             v_id_reg:=substr(p_linha,col_id_reg.inicio,col_id_reg.fim-col_id_reg.inicio+1);
           else
             v_id_reg:=col_id_reg.valor;
           end if;
         end if;
...
--             err_msg:='O tamanho do campo '||to_char(i)||' é maior que 60 caracteres';
         ...
         begin
           ...
           if col_tipofreq.numero is not null then
             ...
             begin
           	   v_tipofreq:=nvl(col_tipofreq.valor, ltrim(rtrim(t_campo(nvl(col_tipofreq.ordem,0)))));


		         exception
		           when value_error then
		              err_msg:='O tamanho do campo TIPOFREQ é maior que 20 caracteres';
		              return(err_msg);
		         end;

			       -- Verificar se existe a frequencia ---------------
	           select count(1) into v_existe_freq
	             from codigos_freq
	            where tipofreq = v_tipofreq;
	           --
	           if v_existe_freq = 0 then
	               err_msg:='Tipo de frequencia não cadastrada: ('||v_tipofreq||')';
           ...
           if col_codfreq.numero is not null then
           	begin
                v_codfreq:=nvl(col_codfreq.valor, ltrim(rtrim(t_campo(nvl(col_codfreq.ordem,0)))));
             exception
               when value_error then
                 err_msg:='O tamanho do campo CODFREQ é maior que 3 caracteres';
                 return(err_msg);
             end;
	           ...
	           select count(1) into v_existe_freq
	             from codigos_freq
	            where codigo  = v_codfreq;
	           --

	           if v_existe_freq = 0 then
	               err_msg:='Frequencia não cadastrada: ('||v_tipofreq||')';
	               return(err_msg);
	           end if;
           end if;
           ...
           v_quantidade:=nvl(col_quantidade.valor,t_campo(nvl(col_quantidade.ordem,0)));
           v_obs:=nvl(col_obs.valor,t_campo(nvl(col_obs.ordem,0)));
           v_pontpubl:=nvl(col_pontpubl.valor,t_campo(nvl(col_pontpubl.ordem,0)));
           v_pontlei:=nvl(col_pontlei.valor,t_campo(nvl(col_pontlei.ordem,0)));
           v_emp_codigo:=nvl(col_emp_codigo.valor,t_campo(nvl(col_emp_codigo.ordem,0)));
           v_hora_entrada:=nvl(col_hora_entrada.valor,t_campo(nvl(col_hora_entrada.ordem,0)));
           v_hora_saida:=nvl(col_hora_saida.valor,t_campo(nvl(col_hora_saida.ordem,0)));
           v_id_reg:=nvl(col_id_reg.valor,t_campo(nvl(col_id_reg.ordem,0)));
           ...
           if p_modo_carga <> 'D' then
               ...
               --
               ------------------------------------------------------------------------------                                             --
               -- Verifica se ja existe na tabela frequencia o registro a ser inserido
               ------------------------------------------------------------------------------
               -- /// Verificar regra de duplicidade para frequencia

                 select count(*)
                   into v_existe_dupl
                             from frequencias f
                            where f.numfunc = v_numfunc
                              and f.numvinc = v_numvinc
                              and f.tipofreq = v_tipofreq
                              and f.codfreq = v_codfreq
                              and f.dtini = to_date(v_dtini, 'DD/MM/YYYY')
                              and f.dtfim = to_date(v_dtfim, 'DD/MM/YYYY');

               if v_existe_dupl > 0 then
               ...
               --
               ------------------------------------------------------------------------------                                             --
               -- Verifica se ja existe frequencia vigente para o funcionário
               ------------------------------------------------------------------------------
                 select count(*)
                   into v_existe_freq_numfunc
                             from frequencias f
                            where f.numfunc = v_numfunc
                              and f.numvinc = v_numvinc
                              and f.tipofreq = v_tipofreq
               ...
               if v_existe_freq_numfunc > 0 then
                  err_msg := 'Frequência já existente para o funcionário' || v_numfunc;
                  return (err_msg);
               end if;
               --
     ...
     begin
       p_rec.codfreq:=ltrim(rtrim(v_codfreq));
     exception
       when others then return('Valor inválido para CODFREQ');
     end;
     begin
     ...
     begin
     	 p_rec.tipofreq:=ltrim(rtrim(v_tipofreq));
     exception
       when others then return('Valor inválido para TIPOFREQ');
     end;
     begin
       p_rec.quantidade:=ltrim(rtrim(v_quantidade));
     exception
       when others then return('Valor inválido para QUANTIDADE');
     end;
     begin
     	  p_rec.pontpubl:=ltrim(rtrim(v_pontpubl));
     exception
       when others then return('Valor inválido para PONTPUBL');
     end;
     begin
     ...
     begin
     	 v_pontlei := ltrim(rtrim(v_pontlei));
     exception
       when others then return('Valor inválido para PONTLEI');
     end;
     begin
     ...
     begin
     	 p_rec.hora_entrada:=ltrim(rtrim(v_hora_entrada));
     exception
       when others then return('Valor inválido para HORA_ENTRADA');
     end;
     begin
       p_rec.hora_saida:=ltrim(rtrim(v_hora_saida));
     exception
       when others then return('Valor inválido para HORA_SAIDA');
     end;
     begin
       p_rec.id_reg:=ltrim(rtrim(v_id_reg));
     exception
       when others then return('Valor inválido para ID_REG');
     end;
