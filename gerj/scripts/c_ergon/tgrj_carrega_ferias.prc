CREATE OR REPLACE 
PROCEDURE TGRJ_CARREGA_FERIAS(P_ARQ IN BLOB, --default fileUpload
...
BEGIN
  ...
  IF p_acao = 3 AND P_ID_EXEC_AUDIT IS NULL THEN
    p_mens := 'Para desfazer a carga de férias, o número da auditoria do processo de carga deve ser fornecido.';
    RETURN;
  END IF;
  ...
  begin
    ...
    select ID_TIPO_ROT
     ...
     WHERE SIGLA = 'Carga de férias';

  EXCEPTION
    when no_data_found then
      insert into HAD_TIPO_ROTINA_EXEC (SIGLA,DESCRICAO) values ('Carga de férias', 'Carga de férias');
  ...
  if p_acao = 1 then
    ...
    v_arq:= 'DECLARE'||chr(10)||
            'BEGIN'||chr(10)||
              'TGRJ_PACK_CARGA_FERIAS.VERIFICA(''' ||
                                                 v_nome            || ''',''' ||
                                                 p_tipo            || ''',''' ||
                                                 P_CAMINHO         || ''',' ||
                                                 P_LAYOUT          || ');' ||CHR(10)||
  ...
  if p_acao = 3 then
    ...
    v_arq:= 'DECLARE'||chr(10)||
            'BEGIN'||chr(10)||
              'TGRJ_PACK_CARGA_FERIAS.DESFAZ_CARGA(''' ||
                                                    v_nome            || ''',''' ||
                                                    p_tipo            || ''',''' ||                                                    
                                                    P_CAMINHO         || ''',' ||
                                                    P_LAYOUT          || ',' ||
